...
If `--index-enabled` is changed to `false` from `true`, MetalGo won't start as doing so would cause a previously complete index to become incomplete, unless the user explicitly says to do so with `--index-allow-incomplete`. This protects you from accidentally running with indexing disabled, after previously running with it enabled, which would result in an incomplete index.

This document shows how to query data from MetalGo's Index API. The Index API is only available when running with `--index-enabled`.

## Go Client

There is a Go implementation of an Index API client. See documentation [here](https://pkg.go.dev/github.com/MetalBlockchain/metalgo/indexer#Client). This client can be used inside a Go program to connect to a MetalGo node that is running with the Index API enabled and make calls to the Index API.
...
}' -H 'content-type:application/json;' https://api.metalblockchain.org/ext/bc/X
```
        ...
        "outputs": [
          {
            ...
            "output": {
              "addresses": ["X-metal1wst8jt3z3fm9ce0z6akj3266zmgccdp03hjlaj"],
              "amount": 4999000000,
              "locktime": 0,
              "threshold": 1
            }
          },
          {
            ...
            "fxID": "spdxUxVJQbX85MGxMHbKw1sHxMnSqJ3QBzDyDYEP3h6TLuxqQ",
            "output": {
              "addresses": ["X-metal1slt2dhfu6a6qezcn5sgtagumq8ag8we75f84sw"],
