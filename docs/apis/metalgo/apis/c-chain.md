---
description: More information available regarding Metal Go APIs and learning how to interact with the C-Chain.
sidebar_position: 4
---
...
:::info
Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to Metal’s view of networkID and chainID and are purely internal to the [C-Chain](../../../overview/getting-started/intro.md#contract-chain-c-chain). On Mainnet, the C-Chain uses `1` and `381931` for these values. On the Tahoe Testnet, it uses `1` and `381932` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods.
