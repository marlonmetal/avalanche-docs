...
METAL plays the same role on the C-Chain that ETH does on the Ethereum Network. When you create or call a smart contract, you pay the transaction fee (gas cost) with METAL. You can transfer METAL between accounts and send METAL to a smart contract using native EVM tools and libraries.
...
- **`gasPrice`** Scalar value equal to the number of Wei (1 Wei = 10^-18 METAL) paid per unit of gas to execute this transaction.
...
- **`value`** Scalar value of native asset (METAL), in Wei (1 Wei = 10^-18 METAL), to be transferred to the message call's recipient or in the case of a contract creation, as an endowment to the newly created contract.
...
```text
curl --location --request POST 'https://api.metalblockchain.org:443/ext/bc/C/rpc' \
...
`nativeAssetBalance` is a precompiled contract at address `0x0100000000000000000000000000000000000001`. `nativeAssetBalance` is the ANT equivalent of using `balance` to get the METAL balance.
